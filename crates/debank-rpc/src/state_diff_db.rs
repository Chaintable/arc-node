...
impl<ExtDB: DatabaseCommit> DatabaseCommit for StateDiffTraceDB<ExtDB> {
    // D16 (revm 34 vs revm 36 divergence): revm 34's `DatabaseCommit::commit` takes
    // `HashMap<Address, Account>` (default hasher); revm 36 takes `AddressMap<Account>`
    // (FbBuildHasher<20>). Tempo's source uses the latter; adapted here.
