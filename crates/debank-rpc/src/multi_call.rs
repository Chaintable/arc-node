...
use reth_rpc_eth_api::{helpers::EthCall, EthApiTypes, RpcTxReq};
            ...
            .spawn_with_state_at_block(target_block, move |eth_api, mut db| {
                ...
                for request in requests {
                    ...
                    if fast_fail
                        && result_response
                            .last()
                            .is_some_and(|r| r.code != MultiCallErrorCode::Success as i32)
                    {
                    ...
                    let gas_used = execute_result.result.tx_gas_used();

                    let mut res = match execute_result.result {
                        ExecutionResult::Success { output, .. } => SingleCallResult {
                        ...
                        ExecutionResult::Revert { output, .. } => SingleCallResult {
                        ...
                        ExecutionResult::Halt { reason, .. } => SingleCallResult {
