...
// DeBank trace conversion has many bounded index and gas counters. Overflow
// would require an impossible block with more than `usize::MAX` trace members
// or `u64::MAX` gas, so keep the workspace arithmetic lint disabled here and
// use checked conversion at external integer-width boundaries.
