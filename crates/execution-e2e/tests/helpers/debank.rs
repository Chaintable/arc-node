...
use arc_execution_config::{
    chainspec::{ArcChainSpec, LOCAL_DEV},
    hardforks::{ArcHardfork, ARC_MAINNET_HARDFORKS},
};
use arc_execution_e2e::{Action, ArcEnvironment};
...
use super::constants::{SYSTEM_ADDRESS, TRANSFER_EVENT_SIGNATURE};

/// Localdev genesis with the hardfork set currently active on Arc mainnet.
pub fn current_mainnet_test_chain_spec() -> Arc<ArcChainSpec> {
    let mut chain_spec = LOCAL_DEV.as_ref().clone();
    chain_spec.inner.hardforks = ARC_MAINNET_HARDFORKS.clone();

    let flags = chain_spec.get_hardfork_flags(0, 0);
    ...
    assert!(chain_spec.is_osaka_active_at_timestamp(0));

    Arc::new(chain_spec)
}

type BlockAssertion = Box<
    dyn Fn(&ArcEnvironment, &DebankOutPut, &BlockStorageDiff, &DebankTransaction) -> Result<()>
        + Send
        + Sync,
>;

/// Fetches the current block through `trace_debankBlock`, checks invariants shared by every
/// scenario, then runs a scenario-specific assertion.
pub struct AssertDebankBlock {
    tx_name: String,
...
impl AssertDebankBlock {
    pub fn new(
        ...
        assertion: impl Fn(&ArcEnvironment, &DebankOutPut, &BlockStorageDiff, &DebankTransaction) -> Result<()>
            + Send
            + Sync
            + 'static,
    ) -> Self {
    ...
    fn execute<'a>(&'a mut self, env: &'a mut ArcEnvironment) -> BoxFuture<'a, Result<()>> {
        Box::pin(async move {
            ...
            let tx_hash = *env.get_tx_hash(&self.tx_name).ok_or_else(|| {
                eyre::eyre!(
                    "transaction '{}' is not in the test environment",
                    self.tx_name
                )
            })?;
            let client = env
            ...
            let transaction = matching_transactions.next().ok_or_else(|| {
                eyre::eyre!("BlockFile does not contain transaction '{}'", self.tx_name)
            })?;
            ensure!(
                matching_transactions.next().is_none(),
                "BlockFile contains transaction '{}' more than once",
                self.tx_name
            );
                ...
                .chain(&output.block_file.error_traces)
            {
                    ...
                    trace.parent_trace_id.is_empty()
                        || trace_ids.contains(trace.parent_trace_id.as_str()),
            ...
            (self.assertion)(env, &output, &state_diff, transaction)
        })
    }
}
