...
use super::helpers::debank::assert_debank_block;
...
async fn run_fixture(tx_name: &'static str, fixture: PrecompileFixture) -> Result<()> {
    ...
    let tx_hash = node
        .send_tx(
            ...
            TransactionRequest {
                ...
                to: Some(TxKind::Call(fixture.target)),
                value: Some(U256::ZERO),
                gas: Some(750_000),
                input: TransactionInput::new(fixture.input.clone()),
                ..Default::default()
            },
        )
        .await?;
    ...
    let expected_status = matches!(&fixture.outcome, ExpectedOutcome::Success(_));
        ...
        "{tx_name} receipt status mismatch"
    );
    ensure!(receipt.logs().is_empty(), "{tx_name} emitted receipt logs");
    ...
    assert_debank_block(
        ...
        move |output, state_diff, transaction| {
            assert_precompile_block(output, state_diff, transaction, &fixture)
        },
    )
    .await
}
